...
def spawn(command: str, cwd = '.') -> str:
    process = subprocess.run(command, cwd=cwd, shell=True, check=False, capture_output=True)
...
def get_version() -> str:
    if isinstance(environment, ROCmEnvironment):
        ...
        assert isinstance(environment, PythonPackageEnvironment)
...
if sys.platform == "win32":
    ...
    def postinstall():
        ...
        if torch.version.hip is None:
            ...
            paths_no_rocm = [path_ for path_ in paths if "rocm" not in path_.lower()]
    ...
    def rocm_init():
        try:
            ...
            class Dummy(type(sys)):
                def __init__(self, *args, **kwargs):
                    super().__init__(kwargs.get("_", ""))
                ...
                def __call__(self, *args, **kwargs):
                    return None
            setattr(torch.distributed, "FileStore", Dummy())
            setattr(torch.distributed, "Store", Dummy())
            setattr(torch.distributed, "TCPStore", Dummy())
            sys.modules["torch._C._distributed_c10d"] = Dummy(_="torch._C._distributed_c10d")

            cholesky_ex_gpu = torch.linalg.cholesky_ex
            @wraps(cholesky_ex_gpu)
            def cholesky_ex(A: torch.Tensor, upper=False, check_errors=False, out=None) -> torch.return_types.linalg_cholesky_ex: # type: ignore
                assert not check_errors
                return_device = A.device
                L = torch.from_numpy(np.linalg.cholesky(A.to("cpu").numpy(), upper=upper)).to(return_device) # type: ignore
                info = torch.tensor(0, dtype=torch.int32, device=return_device)
                if out is not None:
                    out[0].copy_(L)
                    out[1].copy_(info)
                return torch.return_types.linalg_cholesky_ex((L, info), {}) # type: ignore
